INSERT INTO `myblog`.`users` (`login`, `pass`) VALUES ('guest', 'guest');


INSERT INTO `myblog`.`articles`
(
`title`,
`text`,
`date`)
...
(
'Ан-225 «Мрия»',
'Ан-225 «Мрия» (укр. мрія — «мечта», внутреннее обозначение: изделие 402, по кодификации НАТО: Cossack — рус. казак) — транспортный самолёт сверхбольшой грузоподъёмности разработки ОКБ им. О. К. Антонова. Самый большой в мире самолет. Уникальный транспортный самолёт был спроектирован и построен на Киевском Механическом Заводе в 1984—1988 годах. Первый полёт был совершён 21 декабря 1988 года. Изначально было заложено 2 машины, в настоящее время один экземпляр находится в лётном состоянии и эксплуатируется украинской компанией Antonov Airlines. Второй экземпляр готов на 70 %.
...
VALUES
(
'Moustiers Sainte-Marie. La ville de rêves',
'<h2>
